...
<body style="height: 98vh; width: 100vw; background-image: url(https://www.bmmigroup.com/wp-content/uploads/2019/05/food-waste-website-01.png);
 ...
 display: flex; flex-direction: column; align-items: center; justify-content: space-evenly; overflow: hidden;">
    ...
    <div id="container" style="height: 80%; width: 80%; display: flex; align-items: center; justify-content: center;">
        <div class="login-form" style="height: 70%; width: 32%; border-radius: 10px; background: rgba(255, 255, 255, 0.1); 
        backdrop-filter: blur(15px); -webkit-backdrop-filter: blur(15px); transition: transform 0.3s ease-in-out;
         display: flex; flex-direction: column; align-items: center;">
            <h1 style="text-align: center; font-family: 'Courier New', Courier, monospace; font-size: 2.5rem;">LOGIN
            </h1>
            @if (session('error'))
                <div class="alert alert-danger">{{ session('error') }}</div>
            @endif
            <form action="{{ route('login') }}" method="post" style="height: 65%; width: 80%; display: flex; flex-direction: column; 
            align-items: center; justify-content: space-around;">
                ...
                <input type="password" name="password" id="pass" placeholder="Password"
                    style="height: 35px; width: 90%; border-radius: 10px; border: none; padding-inline: 10px; position: relative;"
                    required>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                    style="position:absolute; right: 60; margin-top: 0px;" fill="currentColor" class="eye-slash"
                    viewBox="0 0 16 16">
                    <path
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                    style="position:absolute; display: none; right: 60; margin-top: 00px;" fill="currentColor"
                    class="eye" viewBox="0 0 16 16">
                    <path
                        d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z" />
                    <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0" />
                </svg>
                <button type="Submit"
                    style="height: 35px; width: 80px; border: none; border-radius: 10px; font-family: monospace;">Login</button>
            </form>
            <span style="color: #555;">Don't have an account? <a href="{{ route('register') }}"
                    style="width: 100%; color: #555;">Create an account</a></span>
        </div>
    </div>
</body>
<script>
